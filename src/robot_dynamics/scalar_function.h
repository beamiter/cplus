...
void jacobian(FunctionSignature sig, DiffMethod diff,
              const ScalarFunction<KP> *fun, O J, P y, KP z) {
  CHECK(length(y) == 1);
...
template <typename KP, typename G>
void gradient(DiffMethod diff, const ScalarFunction<KP> *fun, G grad, KP z) {
  if (DiffMethod::UserDefined == diff) {
    gradient(fun, grad, z);
  }
}
template <typename KP, typename G>
void gradient(const AbstractFunction<KP> *fun, G grad, KP z) {
